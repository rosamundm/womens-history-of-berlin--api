...
from streets.views import DistrictViewSet, StreetViewSet, StreetMapViewSet, TagViewSet
...
def test__get_district_list(api_request, api_url, api_user):
    ...
    for i in range(0, 3):
        assert lambda n: "Chärlöttenbürg-Wilmersdörf %d" % n == data[i]["name"]
        assert (
            lambda n: "chaerloettenbuerg-wilmerssdoerf %d" % n
...
def test__get_district_instance(api_request, api_url, api_user):
    ...
    assert lambda n: "chaerloettenbuerg-wilmerssdoerf %d" % n == data["district_slug"]
...
def test__get_street_list(api_request, api_url, api_user):
    ...
    for i in range(0, 3):
        for street in streets:
            assert lambda n: "Rösä-Lüxembürg-Sträße %d" % n == data[i]["name"]
            assert (
                lambda n: "roesae-luexembuerg-straesse %d" % n == data[i]["street_slug"]
            )
            assert "https://www.openstreetmap.org/way/109819106" == data[i]["map_link"]
...
def test__get_street_instance(api_request, api_url, api_user):
    ...
    assert "<p>Ipsum suspendisse ultrices gravida</p>" == data["eponym_description"]
...
def test__district_is_read_only__put(api_request, api_url, api_user):
    ...
    request = api_request.put(
        f"{api_url}districts/",
        content_type="application/json",
        data=json.dumps({"name": "Mitte", "image_path": "picture.jpeg"}),
    )
...
def test__district_is_read_only__delete(api_request, api_url, api_user):
    ...
    request = api_request.delete(
        f"{api_url}districts/", content_type="application/json"
    )
...
def test__street_is_read_only__put(api_request, api_url, api_user):
    ...
    request = api_request.put(
        f"{api_url}streets/",
        content_type="application/json",
        data=json.dumps({"name": "Neuer-Name-Straße", "image_path": "picture.jpeg"}),
    )
...
def test__street_is_read_only__delete(api_request, api_url, api_user):
    ...
    request = api_request.delete(f"{api_url}streets/", content_type="application/json")
...
# @pytest.mark.django_db
# def get_streets_for_map(api_request, api_url, api_user):

# street_entry_complete = StreetFactory.create(entry_complete=True)
# street_entry_not_complete = StreetFactory.create(entry_complete=False)

# view = StreetMapViewSet.as_view(actions={"get": "list"})

# request = api_request.get(f"{api_url}streets-map/")
